...
def test_drum_pad_dummy_device_generate_event():
    ...
    assert d.matchEvent(d.getEventForDrumPad(5, 5, 0)).coordinate == (5, 5)
    assert d.matchEvent(d.getEventForDrumPad(5, 4, 0)).coordinate == (5, 4)
    assert d.matchEvent(d.getEventForDrumPad(8, 0, 0)) is None
...
def test_drum_pad_dummy_device_generate_event_value():
    ...
    assert floatApproxEqRatio(
        0.5,
        d.matchEvent(d.getEventForDrumPad(5, 5, 0.5)).value
    )
