...
def test_drum_pad_dummy_device_generate_event():
    ...
    assert d.matchEvent(getEventForDrumPad(5, 5, 0)).coordinate == (5, 5)
    assert d.matchEvent(getEventForDrumPad(5, 4, 0)).coordinate == (5, 4)
    assert d.matchEvent(getEventForDrumPad(8, 0, 0)) is None
