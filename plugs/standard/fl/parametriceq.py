...
class ParametricEq(StandardPlugin):
    ...
    def tick(self, index: GeneratorIndex):
        if len(self._levels):
            for color, (f, i) in enumerate(zip(self._levels, range(LEVEL, LEVEL+NUM_PARAMS))):
        ...
        if len(self._frequencies):
            for color, (f, i) in enumerate(zip(self._frequencies, range(FREQUENCY, FREQUENCY+NUM_PARAMS))):
                f.annotation = plugins.getParamName(i, *index)
                f.color = COLORS[color]
                f.value = plugins.getParamValue(i, *index)
        if len(self._frequencies):
            for color, (f, i) in enumerate(zip(self._bandwidths, range(BANDWIDTH, BANDWIDTH+NUM_PARAMS))):
                f.annotation = plugins.getParamName(i, *index)
                f.color = COLORS[color]
                f.value = plugins.getParamValue(i, *index)
    ...
    def frequencies(self, control: ControlShadowEvent, index: GeneratorIndex, *args: Any) -> bool:
        ...
        return True
    @eventfilters.toEffectIndex
    def bandwidths(self, control: ControlShadowEvent, index: GeneratorIndex, *args: Any) -> bool:
        plugins.setParamValue(control.value, BANDWIDTH + control.getShadow().coordinate[1], *index)
        return True
